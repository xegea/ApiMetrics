...
function LoadTestsExecutionsPage() {
  ...
  const [selectedOS, setSelectedOS] = useState<'windows' | 'unix'>('unix');
  const [selectedWindowsShell, setSelectedWindowsShell] = useState<'gitbash' | 'powershell' | 'cmd'>('gitbash');
  ...
  const openCommandModal = (executionName: string, filename?: string, instructions?: any) => {
    setCommandModal({
      isOpen: true,
      command: '', // Will be set dynamically based on OS selection
      executionName,
  ...
  const getCommandForOS = (os: 'windows' | 'unix', windowsShell?: 'gitbash' | 'powershell' | 'cmd') => {
    const filename = commandModal.actualFilename || 'execution-plan-*.json';
    
    switch (os) {
      case 'windows':
        const shell = windowsShell || selectedWindowsShell;
        switch (shell) {
          case 'gitbash':
            return `npx @xegea/apimetrics-cli execute-plan --env local ~/Downloads/${filename}`;
          case 'powershell':
            return `npx @xegea/apimetrics-cli execute-plan --env local $env:USERPROFILE\\Downloads\\${filename}`;
          case 'cmd':
            return `npx @xegea/apimetrics-cli execute-plan --env local %USERPROFILE%\\Downloads\\${filename}`;
        }
        break;
      case 'unix':
        // For macOS and Linux - use ~/Downloads
        return `npx @xegea/apimetrics-cli execute-plan --env local ~/Downloads/${filename}`;
      default:
        return `npx @xegea/apimetrics-cli execute-plan --env local Downloads/${filename}`;
    }
  ...
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(getCommandForOS(selectedOS));
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-xl w-full my-8">
            ...
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <h2 className="text-lg font-bold text-gray-800">Run Load Test</h2>
              <button
            ...
            <div className="p-4 space-y-3">
              ...
              {commandModal.actualFilename && (
                <div className="bg-blue-50 border border-blue-200 rounded p-2">
                  <p className="text-xs text-blue-900">
                    <strong>Downloaded:</strong> <code className="text-xs">{commandModal.actualFilename}</code>
                  </p>
                </div>
              )}
              ...
              {/* OS Selection */}
              <div>
                <p className="text-sm text-gray-700 mb-2">Where will you run the test?</p>
              ...
              {selectedOS === 'windows' && (
                <div>
                  <p className="text-xs text-gray-600 mb-1.5">Select your terminal:</p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setSelectedWindowsShell('gitbash')}
                      className={`flex-1 px-2 py-1.5 text-xs rounded border transition-all ${
                        selectedWindowsShell === 'gitbash'
                    ...
                    <button
                      onClick={() => setSelectedWindowsShell('powershell')}
                      className={`flex-1 px-2 py-1.5 text-xs rounded border transition-all ${
                        selectedWindowsShell === 'powershell'
                          ? 'border-blue-500 bg-blue-50 text-blue-700 font-medium'
                          : 'border-gray-300 bg-white text-gray-600 hover:border-gray-400'
                      }`}
                    >
                    ...
                    <button
                      onClick={() => setSelectedWindowsShell('cmd')}
                      className={`flex-1 px-2 py-1.5 text-xs rounded border transition-all ${
                        selectedWindowsShell === 'cmd'
                          ? 'border-blue-500 bg-blue-50 text-blue-700 font-medium'
                          : 'border-gray-300 bg-white text-gray-600 hover:border-gray-400'
                      }`}
                    >
              ...
              {/* Command */}
              <div>
                <p className="text-sm text-gray-700 mb-2">Copy and run in terminal:</p>
                <div className="bg-gray-900 rounded p-3 font-mono text-xs text-gray-100 flex items-center justify-between gap-2">
                  <code className="flex-1 break-all">{getCommandForOS(selectedOS)}</code>
                  <button
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded p-2">
                <p className="text-xs text-yellow-800">
                  Results upload automatically when complete. Requires Node.js/npm.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2 p-4 border-t border-gray-200">
              <button
                onClick={() => setCommandModal({ ...commandModal, isOpen: false })}
                className="flex-1 px-3 py-2 text-sm bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors"
              >
                Close
              </button>
              <button
                onClick={copyToClipboard}
                className="flex-1 px-3 py-2 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors flex items-center justify-center gap-1"
              >
                <FileCopyIcon style={{ fontSize: 16 }} />
                Copy
              </button>
